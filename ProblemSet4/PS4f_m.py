    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            if not isValidWord(strUser, hand, wordList):
...
def playGame(wordList):
    ...
    while check:  #keep playing until exit
        userInput = raw_input('Enter n to deal a new hand, r to replay the last hand, or e to end game: ')
       
        if userInput == "n":
            hand = dealHand(HAND_SIZE)
            playHand(hand, wordList, HAND_SIZE)
            strLastHand = hand #remember last hand for option 'r'

        elif userInput == "r":
            if strLastHand:
                playHand(strLastHand, wordList, HAND_SIZE)
            else:  #first round, user still do not have play a round
                print "You have not played a hand yet. Please play a new hand first!"

        elif userInput == 'e':  #exit the game
            check = False

        else:  #user fail to enter right command (n, r, e)
